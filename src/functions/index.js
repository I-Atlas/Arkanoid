import { Ball, Paddle, Brick, Enhancement } from "./components";
...
export const paddleUpdate = (delta) => {
  ...
  for (let i = enhancements.length - 1; i >= 0; i--) {
    if (
      enhancements[i].x + enhancements[i].width / 2 >
        paddle.x - paddle.width / 2 &&
      enhancements[i].x - enhancements[i].width / 2 <
        paddle.x + paddle.width / 2 &&
      enhancements[i].y + enhancements[i].height / 2 >
    ...
    for (let j = 0; j < game.BRICK_COLUMN; j++) {
      if (bricks[i][j] != null && bricks[i][j].collision(ball)) {
        ...
        if (Math.random() < game.ENHANCEMENT_CHANCE) {
          ...
          let enhancementKey =
            enhancementKeys[Math.floor(Math.random() * enhancementKeys.length)];
